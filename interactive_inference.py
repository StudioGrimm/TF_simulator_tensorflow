...
class MultiHeadedAttention(layers.Layer, VisualWrapper):
    ...
    def call(self, query, key, value, mask=None, training=None):
        ...
        self.save_data(text=f"Dan wendet man wiederholt den Aufmerksamkeitsmechanismus auf das Embedding des Modells an", 
                       x = att_out, 
                       mode_x="color_bar",
...
#inference_model.print_results(input, output)
...
# </head>
# 
# <div style="width: auto; height: 1000px; overflow: auto;">
#   <figure id="fig:transformer"; style="width:100%; height:auto;";>
#     <img src="./img/tf_arch_full.jpg" alt="Gesamtarchitektur eines Transformer-Netzwerks, wie in dem bahnbrechenden Papier Attention Is All You Need dargestellt"/>
#     <figcaption>Abbildung 1: Architektur eines Transformer-Netzwerks</figcaption>
#   </figure>
# </div>
# Da ich Ihnen die Architektur des Transformer-Modells aus der Perspektive seiner Implementierung vorstellen möchte, habe ich versucht, die Architektur in <a href="#fig:transformer">Abbildung 1</a> auf viel detailliertere Weise darzustellen, als dies normalerweise der Fall ist. Wie Sie sehen können, handelt es sich um ein komplexes neuronales Netzwerk, das jedoch nach Erklärung einiger zentraler Konzepte gar nicht so schwer zu verstehen ist. Wir können verschiedene Teile des Netzwerks unterscheiden und sie Schritt für Schritt erklären.
# 
# # Eingabe- und Ausgabepipeline
# 
...
# <br>
# 
# In schwarz werden die einzelnen Module dargestellt, die Verarbeitungsschritte vornehmen. Blaue Symbole kennzeichnen die dabei in den einzelnen Zwischenschritten erzeugten Daten. Gelb dargestellt werden trainierbare Gewichte, die Teil der Eingabepipeline sind.
...
# 
# Der erste Abschnitt eines Transformer-Netzwerks ist die positionelle Einbettung. Sie wird bei jedem Durchlauf des Modells einmal verwendet, um die Eingabe in für den Menschen lesbarem Text in die numerische Tensor-Darstellung zu transformieren, mit der der Transformer die in ihm enthaltenen Informationen verarbeitet. Jede Einbettung besteht aus zwei Schritten dazu kommt ein dritter, der spezifisch für den Transformer ist:
# 
# Warum das und was machen die verschiedenen Schritte?
# 
# ### Tokenisierung
# 
